...
static ssize_t hardware_device_read(struct file *filp, char __user *buf, size_t len, loff_t *off)
{
    ...
    bytes_to_copy = (len < BUF_LEN) ? len : BUF_LEN;

    if(copy_to_user(buf, buffer, bytes_to_copy)) {
        return -EFAULT;
    }
...
static long hardware_device_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    switch(cmd) {
        case HARDWARE_DEVICE_HALT:
            is_halt = true;
            printk(KERN_INFO "Hardware-Device: HARDWARE_DEVICE_HALT\n");
            return 0;

        case HARDWARE_DEVICE_RESUME:
            is_halt = false;
            printk(KERN_INFO "Hardware-Device: HARDWARE_DEVICE_RESUME\n");
            return 0;
